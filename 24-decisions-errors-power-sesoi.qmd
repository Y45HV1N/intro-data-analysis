---
title: "Decisions, errors, power, and SESOI"
---
...
ggplot(df_plot, aes(x, y, color = curve)) +
  geom_ribbon(data = df_typeII, aes(x = x, ymin = 0, ymax = y),
              fill = "#EF4444", alpha = 0.6, inherit.aes = FALSE) +
  geom_ribbon(data = df_typeI, aes(x = x, ymin = 0, ymax = y),
              fill = "#F59E0B", alpha = 0.6, inherit.aes = FALSE) +
  geom_ribbon(data = df_power, aes(x = x, ymin = 0, ymax = y),
              fill = "#10B981", alpha = 0.45, inherit.aes = FALSE) +
  ...
  scale_color_manual(values = c("H0 (true effect = 0)" = "#3B82F6",
                     ...
                     name = NULL) +
  annotate("text", x = 3.0, y = 0.04, label = "Type I error (alpha)",
           color = "#F59E0B", fontface = "bold") +
  annotate("text", x = 1.2, y = 0.04, label = "Type II error (beta)",
           color = "#EF4444", fontface = "bold") +
  annotate("text", x = 4.6, y = 0.20, label = "Power (1 - beta)",
           color = "#10B981", fontface = "bold") +
  labs(title = "Type I error, Type II error, and power, visualized",
       subtitle = "Dashed line = decision threshold; areas correspond to long-run rates",
...
## What is missing from this picture: SESOI
...
We would need to weigh about 22 brownies to have 80% power to detect a true 5-gram shortfall.

### Why "80% power" is meaningless without SESOI
...
### How SESOI changes the interpretation of results
...
## Equivalence testing: a quick look
...
2. **Anchor n to SESOI.** Choose the smallest effect size that would matter, and compute the required sample size from a power analysis.
3. **Run the test once.** No peeking, no optional stopping, no swapping the analysis after seeing the data.
4. **Report the result.** Statistical significance is one piece. Whether the observed effect is larger or smaller than SESOI is another. Confidence intervals make both visible.
...
opts_q6 <- c(
  "Without SESOI the conventional rule of thumb at d = 0.5 still applies, so any claim of 80% power can be interpreted as referring to that medium effect size by default.",
  answer = "Without SESOI the claim does not specify which effect size the power refers to, and two researchers can both say 80% power while one means d = 0.1 and the other means d = 1.0, two designs with completely different implications.",
  "Without SESOI any computed power is purely speculative and cannot be used in any formal way to determine the sample size required for the planned hypothesis test.",
  "Without SESOI the claim still has a precise meaning because the long-run false-positive rate alpha is fixed and the procedure remains calibrated regardless of the effect size."
)
...
opts_q7 <- c(
  ...
  "The effect being smaller than SESOI is irrelevant for the interpretation of the result, since SESOI plays no role once the test has already been run and the p-value has been computed and reported.",
...
opts_q8 <- c(
  "Equivalence testing computes a Bayesian posterior probability that the null hypothesis is exactly true, given the observed data and a prior centered at zero with variance equal to SESOI squared.",
  ...
  answer = "Equivalence testing reframes the question as 'is the effect smaller than SESOI?' rather than 'is the effect different from zero?' and uses two one-sided tests against the SESOI bounds; rejecting both supports a positive claim of practical equivalence.",
...
opts_q10 <- c(
  ...
  answer = "The serious frequentist practice pre-specifies the procedure (test, alpha, design, exclusion rules), anchors the sample size to SESOI, runs the test once as specified, reports the result honestly, and accepts the calibrated long-run error rates as the basis for the inference.",
