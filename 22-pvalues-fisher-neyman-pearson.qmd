...
#| fig.height: 3.5
two_dim <- data.frame(
  Dimension = c("Quantitative", "Qualitative"),
  What = c("A calibrated procedure with controlled long-run error rates.",
           "A predicted claim from a rationale, corroborated or refuted."),
  Promise = c("If you run THIS procedure, repeating it many times, you reject H0 falsely about α of the time.",
              "If the prediction holds up, it is corroborated; if it fails despite adequate power, it is refuted."),
  Property = c("Mathematical / statistical", "Epistemic / about the world")
)
ggplot(two_dim, aes(x = 1, y = -seq_along(Dimension))) +
  geom_text(aes(label = Dimension), fontface = "bold", size = 5, hjust = 0) +
  geom_text(aes(x = 1.5, label = What), hjust = 0, size = 3.7) +
  geom_text(aes(x = 3.5, label = Promise), hjust = 0, size = 3.5) +
  geom_text(aes(x = 5.7, label = Property), hjust = 0, size = 3.5) +
  geom_text(data = data.frame(x = c(1, 1.5, 3.5, 5.7), y = 0,
                              lab = c("Dimension", "What it is",
                                      "What it promises", "Type of claim")),
            aes(x = x, y = y, label = lab),
            fontface = "bold", hjust = 0, size = 4.2, color = "#3B82F6") +
  coord_cartesian(xlim = c(0.9, 7), ylim = c(-2.6, 0.6)) +
  theme_void()
```

We will see in a few sections that the existence of these two dimensions explains a lot of the controversy around exploratory research, "p-hacking," and pre-registration, all of which become subtle once you see that statistical procedures can satisfy Dimension 1 without saying anything about Dimension 2.
...
Mesquida, Warne, and Lakens (2025) develop this point in detail in a sport-science context. They argue that many hypothesis tests in their field, as in psychology, fail to test the hypothesis the authors think they are testing because the predicted claim is not specified precisely enough. They identify five common misalignments between the stated hypothesis and the actual statistical test, and they propose the **PICO framework** (Population, Intervention, Comparator, Outcome) as a tool for stating hypotheses with enough precision that the corresponding test can be derived unambiguously. Though the paper appears in a sport-science venue, Lakens is a psychology methodologist; the issues they document are the same ones that affect psychology and behavioral research more broadly.

The lesson for our purposes: hypotheses do not arrive ready-made for testing. Turning a theoretical idea into a statistical hypothesis takes care, and that care is the substance of what makes a study confirmatory rather than just calibrated. We will return to this in Chapter 25, when we discuss pre-registration as the technology for committing to a predicted claim before data collection.
