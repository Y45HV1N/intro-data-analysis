...
## Going under the hood: how the line is actually computed
...
### A worked example with three students
...
### Why a formula exists at all: the bowl
...
x <- c(1, 2, 3)
y <- c(2, 4, 5)
b0_grid <- seq(-1, 2.5, length.out = 80)
b1_grid <- seq(0, 3, length.out = 80)
grid <- expand.grid(b0 = b0_grid, b1 = b1_grid)
grid$ssr <- mapply(function(b0v, b1v) sum((y - (b0v + b1v * x))^2), grid$b0, grid$b1)
ggplot(grid, aes(x = b0, y = b1, z = ssr, fill = ssr)) +
  geom_raster(interpolate = TRUE) +
  geom_contour(color = "white", alpha = 0.5, linewidth = 0.3) +
  scale_fill_viridis_c(name = "Sum of\nsquared\nresiduals", option = "C") +
  annotate("point", x = 0.67, y = 1.5, color = "red", size = 4) +
  annotate("text", x = 0.67, y = 1.7, label = "OLS line",
           color = "red", fontface = "bold", vjust = 0) +
  labs(x = expression(b[0]~"(intercept)"),
       y = expression(b[1]~"(slope)"),
       title = "The 'bowl' of squared error")
```
...
### What R does with several predictors
...
### Three ways of thinking about the same answer
