...
Section 1 taught you how R works as a tool. Section 2 taught you how to clean, summarize, and visualize a dataset. Section 3 taught you how to fit and interpret linear models, from the simplest one-number model (the mean) all the way to multiple regression with interactions and assumption diagnostics.
...
p_pop <- ggplot(pop_df, aes(x = value)) +
  geom_histogram(binwidth = 0.4, fill = "#9CA3AF", color = "white") +
  coord_cartesian(xlim = c(0, 16)) +
  labs(title = "Population (heavily skewed, not normal)",
       x = "Value", y = "Count")

p_sd <- ggplot(all_means, aes(x = mean, fill = n)) +
  geom_histogram(binwidth = 0.15, alpha = 0.8, color = "white", position = "identity") +
  geom_vline(xintercept = true_mean, color = "#EF4444", linewidth = 0.5, linetype = "dashed") +
  facet_wrap(~ n, nrow = 1) +
  scale_fill_manual(values = c("n = 2" = "#FCA5A5", "n = 5" = "#FBBF24",
                               "n = 30" = "#34D399", "n = 200" = "#60A5FA")) +
  coord_cartesian(xlim = c(0, 16)) +
...
library(grid)
grid::grid.newpage()
grid::grid.draw(rbind(ggplotGrob(p_pop), ggplotGrob(p_sd), size = "last"))
```

The top panel shows the population, which is heavily skewed to the right. Most values are low, but a long tail extends to the right. The bottom panel shows what the sampling distribution of the mean looks like for that population at four sample sizes.
